...
import edu.indiana.iustudentgovernment.controllers.edit.editAdministrative
...
import edu.indiana.iustudentgovernment.models.IusgBranch.ELECTION_COMMISSION
    ...
    route("/edit") {
        get("") {
            ...
            if (user.title.isNotEmpty() && user.title.maxBy { it.rank }!!.rank >= 3) {
                ...
                if (user.branch == ELECTION_COMMISSION || user.isAdministrator()) map["election-commission"] = true
                if (user.isAdministrator()) map["administrator"] = true
